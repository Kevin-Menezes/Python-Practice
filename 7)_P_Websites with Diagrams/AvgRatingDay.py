...
df['Day'] = df['Timestamp'].dt.date # Extracting only date
day_average = df.groupby(['Day']).mean() # New dataframe with only columns that have numbers...coz mean() works on only numbers
print(day_average)
...
def app():
    ...
    hc.options.title.text = "Average Rating by Day" # 'options' gives u to access the string on top and make changes
    
    hc.options.xAxis.categories = list(day_average.index) # Creating a new key in the string
