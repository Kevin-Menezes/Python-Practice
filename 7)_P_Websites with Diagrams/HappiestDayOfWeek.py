...
df['Weekday'] = df['Timestamp'].dt.strftime('%A') # Extracting only weekday names
df['Daynumber'] = df['Timestamp'].dt.strftime('%w') # Extracting weekday number

weekday_average = df.groupby(['Weekday','Daynumber']).mean()
weekday_average = weekday_average.sort_values('Daynumber') # Sorting by the number

weekday_name = weekday_average.index.get_level_values(0) # Getting only names coz it is (week names,week number)
...
def app():
    ...
    hc.options.title.text = "Happiest day of the week" # 'options' gives u to access the string on top and make changes
    
    hc.options.xAxis.categories = list(weekday_name) # Creating a new key in the string
