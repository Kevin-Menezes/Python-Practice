...
df['Week'] = df['Timestamp'].dt.strftime('%Y-%U') # Extracting only date
week_average = df.groupby(['Week']).mean() # New dataframe with only columns that have numbers...coz mean() works on only numbers
print(week_average)
...
def app():
    ...
    hc.options.title.text = "Average Rating by Week" # 'options' gives u to access the string on top and make changes
    
    hc.options.xAxis.categories = list(week_average.index) # Creating a new key in the string
